...
test("a person can circle C on a printed A/B/C/D form and export an editable PDF", async ({
  context,
  page,
}, testInfo) => {
  test.setTimeout(180_000);
  await authenticateTestUser(context, `${testInfo.project.name}-circle-choice`);
  ...
  await saveDraftAndConfirmFor(page, {
    ...
    target: {
      ...
      assertField: field => {
        ...
        expect(field?.definition?.options).toEqual(["A", "B", "C", "D"]);
        expect(field?.definition?.optionMarks).toHaveLength(4);
      },
    },
  });
